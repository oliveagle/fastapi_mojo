...
from openapi_security import operation_security_json, security_schemes_json
...
def _generate_operation(route: Route) raises -> String:
    ...
    # 决策-44/67/69: operation-level security (openapi_security.mojo 单一职责).
    var sec = operation_security_json(h)
    if sec != "":
        sb.append(sec)
...
def generate_openapi(router: Router, title: String, version: String, description: String,
    ...
    for i in range(total):
        ...
        for j in range(total):
            ...
            if not first_method:
                sb.append(",")
            first_method = False
            # OpenAPI 3.0 Path Item method 字段是固定小写关键字 (get/post/...);
            # 上游 FastAPI/Starlette 输出小写 (本仓库路由表内部方法名为大写).
            sb.append("\"" + lower_ascii(router.routes[j].method) + "\":{" + _generate_operation(router.routes[j]) + "}")
        sb.append("}")
    sb.append("}")
    ...
    # 决策-44/69: components = schemas (决策-38) + securitySchemes (有 _auth 路由时)
    var comps = List[String]()
    if len(schemas) > 0:
        comps.append("\"schemas\":{" + ",".join(schemas) + "}")
    # 决策-44/67/69: securitySchemes (openapi_security.mojo) — 路由表所有 _auth 声明.
    var ssj = security_schemes_json(router)
    if ssj != "":
        comps.append(ssj)
    if len(comps) > 0:
