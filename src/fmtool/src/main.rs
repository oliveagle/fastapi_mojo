...
fn main() -> ExitCode {
    ...
    let rc = match sub.as_str() {
        "raw" => run_raw(&rest),
        "cont100" => run_e2e_port("cont100", &rest, |p| e2e::cont100(p)),
        "keepalive" => run_e2e_port("keepalive", &rest, |p| e2e::keepalive(p)),
        "headbody" => run_e2e_port("headbody", &rest, |p| e2e::headbody(p)),
        "ws1" => run_e2e_port("ws1", &rest, |p| e2e::ws1(p)),
        "ws2" => run_e2e_port("ws2", &rest, |p| e2e::ws2(p)),
        "ws3" => run_e2e_port("ws3", &rest, |p| e2e::ws3(p)),
        "ws4" => run_e2e_port("ws4", &rest, |p| e2e::ws4(p)),
