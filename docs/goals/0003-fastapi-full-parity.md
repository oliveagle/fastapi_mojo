...
|---|------|-------------|------|------|------|
| 1 | 路径方法 | GET/POST/PUT/DELETE/PATCH/OPTIONS/HEAD + 405+Allow | ✅ **机制全支持**（bridge opaque method bytes + router 通用匹配；2026-09-11 审计） | **PATCH+body 不解析 = ADR-0014 文档化偏差**（dispatch body = POST/PUT only） | demo PATCH 路由 = P2 cosmetic |
...
| 20 | Pydantic | 嵌套模型/Field 约束/validator/enum/自定义类型 | ✅ 嵌套+Field 约束+enum（决策-38，声明式 spec 等价形态；**无 Pydantic validator closure = Mojo 无闭包，ADR-0014**） | 约束词表扩充（P2：元素级/regex pattern，ADR-0014 §3.5） | §P2 |
