...
- 已覆盖能力（见 §1 矩阵 ✅）：路由/路径参数/查询参数/类型化参数+422/JSON body/
  ...
  **GZip 中间件 (FASTAPI_MOJO_GZIP env 声明式, Rust bridge flate2 纯 Rust, 决策-40 → Starlette 1.6.0 全量对齐 决策-78)**/
...
|---|------|-------------|------|------|------|
| 1 | 路径方法 | GET/POST/PUT/DELETE/PATCH/OPTIONS/HEAD + 405+Allow + **尾斜杠 307** | ✅ **机制全支持**（bridge opaque method bytes + router 通用匹配 + **Starlette `redirect_slashes` 默认行为（决策-71, ADR-0046：未命中时 alt 路径命中（method 无关）→ 307 绝对 `Location` `scheme://host<alt>?<query>`；rstrip 全部尾斜杠；根 `/` 除外；复用决策-68 wire 形态）**；**HEAD 统一仅头无体（决策-77, ADR-0052：上游 HEAD 头同 GET、`Content-Length` = 完整体长度、body 空；`send_response` 单点 `!current_method_is_head()` 抑制，FFI diff=0）**） | **PATCH+body 解析 ✅（决策-57, ADR-0032: dispatch body = POST/PUT/PATCH，闭环 ADR-0014 偏差）** | — |
| 2 | 路径参数 | `{param}` + 类型 + 约束 | ✅ 全量（**参数约束面统一落地 决策-54, ADR-0029**：`_param_constraints` 声明式（`name=gt=3,le=10` / `len=2-4` / `pat=regex` / `mo`（=0 no-op）；**未声明 query 键 = 隐式 str 声明**（len/pat only）；每字段首违 only（数值 mo→ge→gt→le→lt / 字符串 minl→maxl→pat）；422 对象 house 键序 + `ctx` 末位（上游拼写）；**input 类型化 P26-b-8**（在场=raw / 缺失+默认违约=类型化字面量 unquoted / parse 失败=raw）；**typed header**（`_header_types`, 决策-53 面）缺失→默认值校验→422/注入默认字面量, 在场→parse→约束→注入 raw；collect-all 群序 path→query→header；**自研 regex 引擎 `bridge/regex.rs`**（re.search 语义: literal/escape/类/量词/组/alternation/锚/\b; 无反引/环视/命名组/内联标志; 匹配步数上限防 DoS; **FFI diff = +1** `regex_match`）；OpenAPI 3.0.3 约束键（3.0 布尔 exclusiveMinimum / 隐式 str 恒带 type / 字面量原样）；6 demo 路由 /con/*（含 /con/all 三错群序） + **path 百分号解码 决策-80, ADR-0055**（uvicorn 等价: `%XX`→字节 / `%2F` 分段 404 / `+` 非空格 / 非法 UTF-8→U+FFFD） + **lax 标量强制转换 决策-81, ADR-0056**（int/float/bool 字符串 lax + 规范化回显：`007`→7 / `1.50`→1.5 / `yes`→true；整值小数 `2.0`→2；str/enum/标量类型保持 raw）） | 文档化偏差 ×8（ADR-0029 §3.8：① house 键序+ctx 末位 ② str+数值约束注册期拒（优于上游 no-op）③ list+约束注册期拒（优于上游 500）④ 3.0.3 布尔 exclusive 形式 ⑤ regex = 自研 backtracking 子集 ⑥ 隐式 str schema 恒带 type ⑦ 数字字面量原样 ⑧ body 词表扩充 = 矩阵 #4 未来; **矩阵 #3 标量 bool 短消息偏差本决策销账**（完整消息三面对齐, CP-14/24 守护）） | — |
...
| 4 | 请求体 | Pydantic 模型 / dict / 嵌套 | ✅ 声明式 spec（决策-38）+ **pat=REGEX（决策-57, ADR-0032）** + **数组元素级约束（决策-58, ADR-0033：pat/len 逐元素 str[]、ge/le/gt/lt 逐元素 int[]/float[]、items 数组级、422 loc 带 idx、注册期 fail-fast key×类型、OpenAPI 元素级入 items）** + **递归嵌套 JSON Schema（决策-61, ADR-0036：obj{} 与 obj[]{} 全量递归、数组元素 loc/input、OpenAPI recursive object items + min/max）** + **body JSON 跨类型标量强制（决策-82, ADR-0057：int/float/bool 字段及数组元素 pydantic model lax —— `7.0`→7 / `1e2`→100 / `true`→1 / `7.5`→int_from_float / `null`→int_type·float_type·bool_type；数组逐元素规范化重建）** + **body 422 detail 逐字段对齐 + 多字节 body·form 崩溃修复（决策-83, ADR-0058：`ctx`（ctx 末位）+ `mo=N` + 数值首违序 mo→le→lt→ge→gt + str **codepoint** 长度 + 列表 `too_short`/`too_long` + `actual_length` + 单复数 + 短路 + enum `ctx.expected`；`input` 按 JSON 类型渲染（字符串恒带引号）；Mojo `String[byte=]` 多字节 assert 崩溃修复 = form/cookie/body 数组）** + **body Content-Type 分派 + 顶层值语义（决策-85, ADR-0060：上游 `strict_content_type=True` 默认——仅 `application/json` 或 `application/*+json`（ci，忽略 `;` 参数）触发 JSON 解析，否则原始字符串 → `model_attributes_type`；无 body / JSON `null` → `missing`；JSON 非 object → `model_attributes_type`（input 原值）；非法 JSON → `json_invalid` `["body", pos]` + `ctx.error` + input `{}`（CPython json 位置/消息复刻，`body_json.mojo`）；修复 `input` 裸文本 → detail 非法 JSON BUG）** + **Body(embed=True) 请求体嵌入（决策-86, ADR-0061：`_body_embed` 单模型包裹——顶层非 object/键缺失/nil→`missing ["body",<param>]`、键值非 dict→`model_attributes_type`、内层 loc `["body",<param>...]`、OpenAPI `Body_<name>_<method>` 包裹 `$ref`）** + **float 正确舍入解析 + CPython `repr` 等价格式化（决策-87, ADR-0062：修 Mojo `Float64(String)` 长字面量（>~20 位有效数字）抛错 → 误判 `float_parsing` 422 + `String(Float64)` 偶发非最短不往返；新叶 `float_repr.mojo`（纯 Mojo + 既有 libc `atof`/`strfromd`，**FFI diff=0**，零新 crate/Rust/C）——`atof_f64` 正确舍入 + `fmt_f64_repr`（CPython repr 等价：最小往返位数 + Python 记法阈值 + `-0.0`）+ `is_dec_f64_syntax` 文法门；接线 `numlit`/`body_schema`；499,756 随机 double 0 不一致； + **body 422 `input` CPython/Starlette 反序列化-重序列化（决策-88, ADR-0063：上游 `JSONResponse` = `json.dumps(ensure_ascii=False, separators=(",",":"), allow_nan=False)`，pydantic error 的 input 是已反序列化对象→重新序列化；新叶 `json_canon.mojo`（纯 Mojo，**FFI diff=0**）：`canon_json` 去空白/数字规范化（`1.50`→`1.5`/`1e2`→`100.0`/`-0`→`0`）/字符串解码重编码（`\u0041`→`A`/`caf\u00e9`→`café`/`\b` 短转义）/重复键后者胜；接线 `_json_input_frag`/`_json_input_frag_typed`/`_body_input`；差分 fuzz 2961 组 0 不一致；e2e JC-1..JC-12）** | **validator 自定义回调 = Mojo 无闭包（硬边界, ADR-0014）**；约束词表（gt/ge/lt/le/len/items/pat, 标量+数组元素级）= 等价形态；T[][] 仍为文档化边界 | — |
...
| 8 | Cookie | Cookie(...) | ✅ | — | — |
| 9 | 依赖注入 | Depends (嵌套/缓存/安全依赖) | ✅ 全量（决策-47, ADR-0022：默认 cached = 每请求 memo 表（菱形/三重菱形 1 次，值同源，P9-1/5）+ `_depends_nocache` = use_cache=False（P9-2/4）+ 嵌套 nocache 结果入库供 cached 引用复用（P9-3）+ 每请求作用域 + `_dep_calls` 观测超集；APIRouter 对称扩展 `base_deps_nocache`/`include_router(deps_nc=)`；FFI diff = 0） + **`Depends(yield)` teardown 决策-75, ADR-0050**：依赖 `_dep_teardown`（换行分隔命令）实际派发登记（memo 命中不登记 → use_cache=True 每请求一次），响应 flush 后**逆解析序** LIFO 执行（上游 request `AsyncExitStack`；在 background tasks 之后；异常响应后仍执行 = try/finally 形式）；接入点 ×6；**FFI diff=0**） | 文档化偏差 ×5（ADR-0022 §3.5：per-name memo vs per-dependant / _dep_calls 超集 / 基础依赖恒 cached / 解析序先 cached 后 nocache；ADR-0050：teardown = 声明式命令且恒执行（裸后置异常跳过不模拟）） | — |
| 10 | 响应类型 | JSON/HTML/PlainText/File/Streaming/Redirect/ORJSON/UJSON/Response | ✅ 全量（JSON/HTML/SSE 既有 + **Redirect 决策-68, ADR-0043**（307/303/301/308 + `Location`（上游 safe set 编码）+ `Content-Length: 0` + 无 Content-Type + 空 body；HTTP/2 HEADERS-only）+ **File/Streaming 决策-48, ADR-0023**：FileResponse = Range 7 步顺序解析（>100 段 → 200 quirk）/ 206 单段·suffix·open·clamp / multipart（26-hex boundary + CL 闭式 + 重叠合并）/ 400×4 精确消息 / 416（`bytes */size` 空体）/ 500（无文件头）/ If-Range = ETag\|LM / HEAD 仅头 / CD（attachment\|inline RFC5987）/ etag = md5(f64(mtime)-size) / 64KB 块直发；StreamingResponse = TE chunked（no-CT quirk / 自定义 status / extra 头 / 空体键存在语义）；**SSE `ServerSentEvent` 字段（event/id/retry/comment，`format_sse_event` 字段序 comment→event→data→id→retry + `\n\n`、`_split_sse_lines` 保留尾空串、data=raw_data，决策-72, ADR-0047）**；ORJSON/UJSON ≡ json.mojo（既有 F3 决策）；Rust bridge file_protocol 230 + file_serve 416 + FFI ×2，零新 crate，MD5 K 表 const = libm 零化） | 文档化偏差 ×7（ADR-0023 §3.5：① ORJSON≡json.mojo（既有）② HEAD 仅头 vs 上游 405 quirk（APIRoute methods={GET}，更优）③ ~~GZip 不介入 file/streaming（上游压缩，P2）~~ → **已解除（决策-78, ADR-0053：streaming/FileResponse 均可压，对齐 starlette 1.6.0）**④ 整秒 mtime etag `"N"` vs 上游 `"N.0"`（opaque；非整秒逐字节相同）⑤ i64 溢出段 → 400 vs 上游 416（>9.2EB 不可达）⑥ `_file_path` 静态目录相对 + extra 不得覆写 CT/ETag（收窄）⑦ INM·IMS 忽略 = parity（列此完备）） | — |
| 11 | response_model | 只返回声明字段 + include/exclude/none/unset/defaults/by_alias | ✅ include+exclude+exclude_none（决策-35/41, ADR-0016）+ **exclude_unset·exclude_defaults·by_alias（决策-89, ADR-0064：plain 注入声明默认值；unset 剔请求未提供；defaults 剔值等于声明默认；by_alias 输出键真/假；模型字段类型+默认同源 `_body_schema`；int/float/bool/数组/对象渲染 raw JSON number；无 `_response_model` 仍 no-op）** | — | — |
| 12 | 状态码 | status_code 声明 | ✅ | — | — |
| 13 | 异常 | HTTPException/RequestValidationError/自定义 handler | ✅ 全量（HTTPException F2 / 422 F1 既有 + **任意异常类型 handler 决策-49, ADR-0024**：字符串 tag 约定 `raise Error("TAG: msg")`（Mojo 1.0.0 仅 Error 类型, P13-M2/M5）+ 声明式表（env `FASTAPI_MOJO_EXCEPTION_HANDLERS` "TAG:STATUS:BODY[:json];…" 全局 / `_exc_handlers` 路由级整体替换超集 / 同 tag 后者胜 P13-2）+ 声明式 raise 钩子 `_exception_raise`（endpoint body 位置）+ 路由级 try/except guard（= 上游 wrap_app_handling_exceptions；查找 精确 tag → `Exception` catch-all（= ServerErrorMiddleware 500/Exception 键, P13-9）→ 默认 500 "Internal Server Error" text/plain（P13-10 逐字 parity）；body 模板 {exc}/{tag} 插值（json 条目 `_json_escape`）；P13-8 日志 quirk 模拟 + **自定义响应头 决策-74, ADR-0049**：路由级 `_exc_headers` / 全局 `FASTAPI_MOJO_EXCEPTION_HEADERS`（`;` 分隔 `TAG=Name: V|Name2: V2`，命中 tag → `Exception` catch-all；JSON 面复用 `send_simple_response_extra`，text 面新增 `send_text_response_status_extra`，+1 FFI）） | 文档化偏差 ×8（ADR-0024 §3.5：① 无类→字符串 tag（无 MRO）② handler=声明式条目非 callable ③ 无 int status 键（HTTPException 非 raised 异常）④ response_started 路径结构性不可达（单发）⑤ 双层 map→单表 ⑥ 日志 quirk 线形模拟 ⑦ per-route=超集 ⑧ 中间件抛出 gap） | — |
...
| 15 | CORS | CORSMiddleware (origins/methods/headers/credentials/allow_origin_regex/expose_headers/allow_private_network) | ✅ 声明式 env 等价（决策-42/73, ADR-0017/0048：ORIGINS/ORIGIN_REGEX/METHODS/HEADERS/CREDENTIALS/MAX_AGE/EXPOSE_HEADERS/PRIVATE_NETWORK；普通响应 simple_headers 恒发 + echo `Vary: Origin`；**真预检（Origin+ACRM）200 `OK` / 400 `Disallowed CORS …` text/plain + 完整头集**；`*` methods→ALL_METHODS / `*` headers→镜像；allow_headers=sorted(SAFELISTED∪配置)；method 匹配大小写敏感) | 裸 OPTIONS / 无 ACRM → 204 通配超集（既有文档化, e2e 守护）；allow_origin_regex 受限于手写 regex 子集 | — |
| 16 | OpenAPI | spec + Swagger + tags/prefix/desc | ✅ 全量（**OpenAPI 精化 决策-52, ADR-0027**：app 级 9 `FASTAPI_MOJO_OPENAPI_*` env（title/version/description/terms/contact/license/servers/tags/external_docs — /openapi.json 请求期读, 畸形 → 省略字段不 500）+ 路由级 8 声明（`_summary` 默认 = name Python title() / `_description` / `_response_description` 默认 "Successful Response" / `_operation_id` 默认 `{name}{path 逐 /{ }→_}_{method}`（P24-6: 上游 `re.sub(\W→_)` 逐字符**无 `_+` 折叠**）/ `_deprecated` / `_include_in_schema`（不进 paths/components 仍可服务）/ `_status_code`="NNN Reason"（wire 仅当结果恰 "200 OK" 覆写 + spec 主键前 3 位）/ `_responses` 额外状态码（重复主键注册期拒绝））；注册期校验 `check_openapi_specs`（check_ws_specs/check_state_specs 同策略 fail-fast）；operation 键序 P24-4（tags? summary? description? operationId ...）+ 根键序 P24-10（openapi, info, servers?, paths, components?, tags?, externalDocs?；externalDocs description 先）；AnyUrl 2.13.5 规范化（contact/license/externalDocs url：host-only 尾 `/`、path 空且有 `?`/`#` 前插 `/`；**servers url 原样透传** = 上游 AnyUrl|str str 优先, P24-12 修正）；3 demo 路由 /meta/{probe,hidden,made}；**FFI diff = 0**（纯 Mojo：openapi_custom.mojo 381 ln + openapi.mojo 497 ln 重写）；**内置文档路由补全 决策-76, ADR-0051**：`/redoc` + `/docs/oauth2-redirect`（GET+HEAD）+ `/docs` `oauth2RedirectUrl`，FFI diff=0） | 文档化偏差 ×9（ADR-0027 §3.5：① 3.0.3 vs 上游 3.1.0（200 schema `{type:object}` vs `{}`）② 无运行期可变 spec（请求期声明式再生成；上游 `extra` 本身 no-op）③ _status_code = 全 status line（_stream_status/_file_status 同型）④ app 级 = env 非构造器（畸形省略）⑤ root_path/openapi_url/docs_url/redoc/webhooks 未实现（P24-11: 0.141.1 root_path 对 spec 无影响）⑥ summary = handler.name title()（同字符串约定）⑦ _responses 重复主键注册期拒绝（上游 dict 覆盖）⑧ 路由 tags 不并入根 tags （P24-2 复刻）⑨ servers url 不规范化（上游 AnyUrl|str, parity）） | — |
| 17 | 安全 | HTTPBasic/HTTPBearer/APIKey/OAuth2/JWT/**HTTPDigest**/**OpenIdConnect**/**OAuth2AuthorizationCodeBearer**/get_current_user/**scopes** | ✅ 全量（决策-34 Basic/Bearer/APIKey + 决策-44 OAuth2/JWT + **决策-67 OAuth2 作用域 (ADR-0042)** + **决策-69 安全 OpenAPI securitySchemes (basic/bearer/digest/apiKey header·query·cookie) + HTTPDigest (ADR-0044)** + **决策-70 OpenIdConnect + OAuth2AuthorizationCodeBearer (ADR-0045)**，ADR-0019：/token password grant（宽松 form 422 全收集）+ JWT HS256（alg 白名单 + exp/nbf/sub）+ get_current_user = sub→auth_user + OpenAPI securitySchemes） | 文档化偏差（空 Bearer → 401 非 403；sub 空串也拒；_auth_users CSV = 凭据声明式等价，ADR-0019 §3.5，e2e 守护） | — |
...
| 24 | 压缩 | GZipMiddleware | ✅ 全量（决策-40, ADR-0015 → **决策-78, ADR-0053 Starlette 1.6.0 全量对齐**：FASTAPI_MOJO_GZIP* + `plan()` 单点判定 + flate2 纯 Rust；Vary 恒加于可压响应 / 大小写敏感 client 判定 / 13 项默认排除表 / streaming+FileResponse 可压 / level 9） | 文档化偏差（ADR-0053 §7：① `GZIP_MAX_SIZE` 非上游内存保护扩展 ② 大 FileResponse（≥64 KiB）单 body 带 CL vs 上游 streaming 无 CL ③ streaming 压缩合并 gzip vs 上游逐块 Z_SYNC_FLUSH（解压等价）④ flate2 vs zlib 字节非逐字节相同（解压一致）⑤ thread_minimum_size 未建模） | — |
...
| 26 | 标量类型 | uuid/date/datetime/time/timedelta/decimal 强制转换 + 422 + OpenAPI format | ✅ 全量（**决策-79, ADR-0054**：6 类 pydantic 内建标量 path/query/header/body/form/list 全场景；纯 Mojo 解析器 `date_types`/`time_types`/`scalar_types`；422 `type`/`msg`/`ctx`/`input` 逐字段对齐上游 pydantic_core 2.46.4；OpenAPI uuid/date/date-time/time/duration/decimal anyOf；byte-safe `bof()` 规避 `String[byte=]` 多字节 assert；**FFI diff=0**；e2e **609→664/664** SC-1..SC-37） | handler 回显不归一化（path/query 校验通过后仍原始字符串，既有 int/float house 行为）/ path 参数不 URL 解码（既有）/ 少量 speedate 畸形分支 / decimal 仅 ASCII 数字 / 参数 schema 无 title（ADR-0054 §3.5） | — |
...
---
*最后更新：2026-09-12（**决策-71 Starlette `redirect_slashes` 等价**（ADR-0046, `fastapi_mojo-kvd` bead 销账）：实现 Starlette Router 默认路由行为 —— 路径未匹配时把尾斜杠取反（`/a/b` ↔ `/a/b/`，`rstrip` 去全部尾斜杠）再试，命中（**path 命中即可，method 无关**，上游 `match != NONE`/PARTIAL 同型）→ **307 Temporary Redirect** 到**绝对 URL** `scheme://host<alt>?<query>`（query 保留）；根 `/` 不重定向；未命中维持 404。新增纯逻辑 `redirect_slashes.mojo`（`alt_slash_path`/`build_redirect_location`/`mojo run` 自检）；dispatch 在未匹配分支接线，**复用决策-68 `send_redirect_response`**（无 Content-Type + `Content-Length: 0` + `Location` + 空 body）；scheme 取 TLS env、host 取 `Host` 头（缺失兜底）。**支撑修复**：fmtool testclient `Host` 无端口 → 绝对 Location 丢端口/follow 连错口，改为非默认端口带端口（对齐 httpx / WS testclient）。**FFI diff = 0**；e2e **567→577/577**（SL-1..SL-10）/ bridge **500/0/4** / fmtool **35/0** / clippy 0（双 crate）/ bench 6 场景 0 errors / binary ≤6 MiB / ldd 仅 libc / env-i 干净启动 / C=Python=orphans 0；边界=内置 `/openapi.json`/`/docs`/`/metrics`/`/traces` 走 router 前特判未纳入）
下一轮：P1/P2 open beads（multi-arch / asgi-shim / SecurityScopes 对象 / Depends yield teardown）
*最后更新：2026-09-12（**决策-70 OpenIdConnect + OAuth2AuthorizationCodeBearer**（ADR-0045, `fastapi_mojo-cz1` bead 销账）：补齐上游 `fastapi.security` 剩余两类 ——①**OpenIdConnect stub parity**：`_auth="openid"` + `_openid_url`，**只校验 Authorization 头存在**（任意 scheme，含 `Basic x`）→ 缺失/空 **401 `Not authenticated` + `WWW-Authenticate: Bearer`**；存在 → ok 注入 `auth_credentials` = **原始头**；OpenAPI `{"type":"openIdConnect","openIdConnectUrl":"<url>"}`；②**OAuth2AuthorizationCodeBearer**：`_auth="authcode"` + `_authcode_authorization_url`/`_authcode_token_url`/`_authcode_scopes`，运行时首个空格切 scheme/param，scheme 大小写不敏感 `bearer`（param 可空）→ 200 + `auth_token`，缺失/非 bearer → 401 + `WWW-Authenticate: Bearer`；OpenAPI `{"type":"oauth2","flows":{"authorizationCode":{"scopes":{...},"authorizationUrl":"..","tokenUrl":".."}}}`；③**键隔离**：`_oauth2_scheme_scopes_json(router, code)` — password 用 `_oauth2_*`、authorizationCode 用 `_authcode_*`，并存时互不污染 tokenUrl/scopes（e2e AC-7 守护）；**FFI diff = 0**；e2e **556→567/567**（OI-1..4 + AC-1..7）/ bridge **500/0/4** / fmtool **35/0** / clippy 0（双 crate）/ bench 6 场景 0 errors / binary **5,024,096 B**（≤6 MiB）/ ldd 仅 libc / env-i 干净启动 / C=Python=orphans 0；边界=openid 不校验 token（上游 stub）、authcode 只提取 Bearer（不校验 JWT）、param 空 → auth_token 不注入）
下一轮：P1/P2 open beads（multi-arch / asgi-shim / SecurityScopes 对象 / redirect_slashes）
*最后更新：2026-09-12（**决策-69 安全 OpenAPI schemes + HTTPDigest**（ADR-0044, security-openapi-digest bead 销账）：① **OpenAPI `components.securitySchemes` 上游形态对齐** — HTTPBasic→`type:http,scheme:basic`；HTTPBearer→`type:http,scheme:bearer`；HTTPDigest→`type:http,scheme:digest`；APIKey(Header/Query/Cookie)→`type:apiKey,in:header|query|cookie,name:<n>`（键序 type,in,name）；oauth2→`type:oauth2,flows.password.{scopes,tokenUrl}`；operation `security:[{<scheme>:[]}]`（非 oauth2 空 scope）；**修复 pre-existing BUG**：`paths` method key 曾为大写 `GET`（OpenAPI 3.0 固定小写字段）导致 Swagger UI 无法渲染任何 operation；② **HTTPDigest 运行时**（`_auth=digest`）— scheme 大小写不敏感 == `digest` 且凭据非空，否则 **401 `{"detail":"Not authenticated"}` + `WWW-Authenticate: Digest`**（无 realm），成功返回 `(scheme 原样, credentials)`；③ 新模块 `openapi_security.mojo`（212 行，从 openapi.mojo 拆出，openapi.mojo 513→444 行 < 500）；④ `_eq_ci` helper + `AuthResult.auth_scheme/auth_credentials` + dispatch 注入 `auth_scheme`/`auth_credentials`；demo `/digest` + `/basic-alt`；**FFI diff = 0**；e2e **545→556/556**（DG-1..DG-6 + SO-1..SO-5，OT-24 断言改为顺序无关）/ bridge **500/0/4** / fmtool **35/0** / clippy 0（双 crate）/ bench 6 场景 0 errors / binary **5,020,000 B**（≤6 MiB）/ ldd 仅 libc / env-i 干净启动 / C=Python=orphans 0 / `find src -name '*.c'`=0；边界=安全依赖未挂载时无 operation security、OpenIdConnect/OAuth2AuthorizationCodeBearer 单独跟踪）
下一轮：P1/P2 open beads（multi-arch / asgi-shim / OpenIdConnect / OAuth2AuthorizationCodeBearer）
---
*最后更新：2026-09-13（**决策-89 response_model exclude_unset/exclude_defaults/by_alias**（ADR-0064, Goal-0003 §1 矩阵 #11 response_model / 兑现 ADR-0062 §8 #3 + ADR-0063 §8 #2）：上游 probe（fastapi 0.141.1）—— plain `response_model` **注入声明默认值**；`response_model_exclude_unset` 剔**请求未提供**字段（显式提供=默认值仍保留）；`response_model_exclude_defaults` 剔**值等于声明默认**字段（未提供/显式均剔）；`response_model_by_alias`（默认 true）决定输出键 alias/字段名。声明式 `_response_exclude_unset`/`_response_exclude_defaults`/`_response_aliases="name=alias;..."`/`_response_by_alias="false"`；模型**字段类型 + 默认值取自 `_body_schema`**（与请求校验同源）；值解析优先 `body_<f>`（校验后规范值）再 `<f>`；int/float/bool/数组/对象渲染 raw JSON（`__nested__:` 直通，float 字段输出 JSON number），str/未知保持 JSON 字符串（既有零变化）。`validate_body_schema` 返回值扩为 **4 元组**（第 4 元 `set_csv` = 显式提供顶层字段名，成功/422 路径均回传；embed 取内层同名集），dispatch 一行透传 `provided_csv`。应用序 include → exclude → unset/defaults → exclude_none；无 `_response_model` 整体 no-op。**FFI diff=0**（纯 Mojo，零 Rust/零 C）；e2e **844→859/859**（RMP-1..3 / RMU-1..5 / RMD-1..4 / RMB-1..3，逐条对齐上游 `/rn`·`/ru`·`/rd`·`/ba`·`/bn`）/ bridge 516/0/4（无 Rust 改动，+7 Mojo 自测断言）/ fmtool 35/0 / clippy 0（双 crate）/ 13 Mojo 自测全绿 / bench 6 场景 0 errors / binary 5,265,856 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0）
下一轮：P1/P2 open beads（非有限 float 渲染 / SecurityScopes 对象 / OAuth2PasswordRequestForm 对象面 / jsonable_encoder / OpenAPI 3.1 / multi-arch / asgi-shim）
*最后更新：2026-09-13（**决策-88 body 422 `input` CPython/Starlette 反序列化-重序列化**（ADR-0063, Goal-0003 §1 矩阵 #4 422 detail / 兑现 ADR-0060 §5 偏差行 + §8 #3）：上游 422 由 Starlette `JSONResponse`（`json.dumps(ensure_ascii=False, separators=(",",":"), allow_nan=False)`）渲染，pydantic error 的 `input` = 已反序列化对象 → 重新序列化；本实现此前回显原始 JSON span（形态差异）。差距：`[1, 2, 3]`→`[1,2,3]`、`1.50`→`1.5`、`1e2`→`100.0`、`"caf\u00e9"`→`"café"`、`"\u0041"`→`"A"`。新叶 `json_canon.mojo`（371 LOC，纯 Mojo，**FFI diff=0**，零新 crate/Rust/C）：`canon_json` 单遍递归下降校验+重序列化（失败/非有限 float/孤立代理→原样回退）+ `_cpython_escape`（`\b\f\n\r\t` + `<0x20`→`\u00XX`）+ `json_string_literal`；数字复用决策-87 `atof_f64`+`fmt_f64_repr`；对象重复键后者胜。接线 `body_constraints._json_input_frag`/`_json_input_frag_typed`（string 分支）+ `body_validate._body_input` + 非 JSON CT 原始串。差分 fuzz（上游 `M{i:int,f:float,b:bool}` `/cross` vs `/validate/cross`）2961 组非规范 JSON `input`/status 0 不一致；e2e **832→844/844**（JC-1..JC-12）/ bridge 516/0/4（无 Rust 改动）/ fmtool 35/0 / clippy 0（双 crate）/ 13 Mojo 自测全绿（+`json_canon_test`）/ bench 6 场景 0 errors / binary 5,220,800 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0）
下一轮：P1/P2 open beads（非有限 float 渲染 / response_model exclude_unset·exclude_defaults·by_alias / SecurityScopes 对象 / OAuth2PasswordRequestForm 对象面 / jsonable_encoder / OpenAPI 3.1 / multi-arch / asgi-shim）
*最后更新：2026-09-12（**决策-87 float 正确舍入解析 + CPython repr 等价格式化**（ADR-0062, Goal-0003 §1 矩阵 #4 请求体数值面 / 兑现 ADR-0061 §8 #2「float 格式化边界」）：修 Mojo 1.0.0 两个真原语缺陷 —— `Float64(String)` 整数部分按 int64 累加 → >~20 位有效数字/长整数部分抛错，把 float64 范围内合法十进制（`12345678901234567890.0`）误判 `float_parsing` 422（上游 = `1.2345678901234567e+19`）；`String(Float64)` 偶发非最短/不往返（`7.531168259201221e+16`→`"7.53116825920122e+16"`）。新叶模块 `float_repr.mojo`（194 LOC，纯 Mojo + 既有 libc 符号 `atof`/`strfromd`，**FFI diff=0**，零新 crate/Rust/C）：`atof_f64`（strtod 正确舍入）+ `fmt_f64_repr`（**CPython repr 等价**：最小 p∈1..17 往返位数 + Python 记法 `decpt<=-4||decpt>16` 科学记数否则定点 + 整值补 `.0` + `-0.0`）+ `is_dec_f64_syntax`（文法门，`atof` 垃圾串返回 0.0 不可当依据；比 Mojo `Float64` 严，后者误收 `"1.2.3"`→`1.23`）。接线 `numlit`（`parse_float_lax`/`parse_f64`/`fmt_num`）+ `body_schema`（`_parse_f64`/`fmt_num`）。原型 499,756 随机 double + 边界 0 不一致；上线 5857 输入 0 不一致。e2e **820→832/832**（LF-1..LF-12）/ bridge 516/0/4（无 Rust 改动，+14 Mojo 自测断言）/ fmtool 35/0 / clippy 0（双 crate）/ 12 Mojo 自测全绿 / bench 6 场景 0 errors / binary 5,200,320 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0）
...
*最后更新：2026-09-12（**决策-86 Body(embed=True) 请求体嵌入语义**（ADR-0061, Goal-0003 §1 矩阵 #4 请求体 / `fastapi_mojo-280` bead 销账）：上游 `Body(embed=True)` 把单一 body 模型包成单字段模型 `{<param>: Model}`，运行期 = `received_body.get(<param>)` 语义 —— 顶层非 object（无 body / 非 JSON CT 原始字符串 / JSON `null` / 数组 / 数字 / bool）或键缺失/nil → `missing ["body", <param>]`（非嵌入时顶层非 object 报 `model_attributes_type`，embed 因原始值无 `.get` 报 `missing`）；键值非 dict → `model_attributes_type` `["body", <param>]`（input 内层原值）；键值 dict → 内层模型校验（loc 前缀 `["body", <param>]`）；非法 JSON 仍 `json_invalid ["body", pos]` + ctx。声明式 `_body_embed`；`body_validate.mojo` 新 `_validate_body_embed`；OpenAPI `Body_<name>_<method>` 包裹 `$ref`。**FFI diff=0**（纯 Mojo，零 Rust/零 C）；e2e **802→820/820**（EB-1..EB-18）/ bridge 516/0/4（无 Rust 改动，+14 Mojo 自测断言）/ fmtool 35/0 / clippy 0（双 crate）/ 12 Mojo 自测全绿 / bench 6 场景 0 errors / binary 5,220,800 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0）
...
*最后更新：2026-09-12（**决策-85 body Content-Type 分派 + 顶层值语义**（ADR-0060, Goal-0003 §1 矩阵 #4 请求体 / `fastapi_mojo-body-content-type-dispatch-iyj` bead 销账；兑现 ADR-0058 §5 偏差行 + §8 后续缺口 #3）：对齐上游 `strict_content_type=True` 默认 —— 仅 `application/json` / `application/*+json`（ci，忽略 `;` 参数）触发 JSON 解析；否则 body = 原始字符串 → `model_attributes_type` `["body"]`（input 带引号原始串）。附修顶层值语义：无 body / JSON `null` → `missing`；JSON 数组/字符串/数字/bool → `model_attributes_type`（input 原值）；非法 JSON → `json_invalid` `["body", pos]` + `ctx.error` + input `{}`。新叶模块 `body_json.mojo`（378 LOC，零 import，全 `as_bytes()` 字节安全）递归下降复刻 CPython `json` 首个错误位置+消息（9 类）+ 数字文法 + allow_nan 常量；`content_type_is_json` 分派；`body_json_test.mojo`（85 LOC）自测。修复真 BUG：此前 `json_invalid` 的 `input` 回显裸文本 → 422 `detail` 非法 JSON。**FFI diff=0**（纯 Mojo，零 Rust/零 C）；e2e **785→802/802**（CT-1..CT-17）/ bridge 516/0/4（无 Rust 改动）/ fmtool 35/0 / clippy 0（双 crate）/ bench 6 场景 0 errors / binary 5,204,416 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0；偏差=input 规范化/键序/错误体超集（ADR-0060 §5）)
下一轮：P1/P2 open beads（非有限 float 渲染 / float 格式化边界 / SecurityScopes 对象 / OAuth2PasswordRequestForm 对象面 / CORS 裸 OPTIONS 超集 / multi-arch / asgi-shim）
*最后更新：2026-09-12（**决策-84 多字节 raw wire 输入字节安全**（ADR-0059, Goal-0003 §1 矩阵 #1/#2/#17 请求处理面 / `fastapi_mojo-multibyte-wire-byte-safety-led` bead 销账；兑现 ADR-0058 §8 后续缺口 #1）：Mojo 1.0.0 `String[byte=i]` 在 continuation 字节下标 `Assert Error` → server 进程 abort（原始非 curl 重编码的多字节/非法字节 wire 输入可远程触发）—— **全请求面字节安全**：path（redirect_slashes / url_decode_path / path 参数）、query、header name+value（含非法 lead 字节 `\xff`，修 `span_to_str` 越界读）、Content-Type media-type、auth（`_starts_with`/`b64_decode`/`_trim`/digest·authcode 空格扫描）、JWT（`_jwt_parts`/`_json_field`）、WS 子协议 offer（`trim_spaces`）、裸多字节 JSON body 标量、typed list header（`split_csv`）、JSON access log（codepoint-aware `_json_escape`）。全部改 `as_bytes()` 原始字节；非法 UTF-8 → U+FFFD（对齐既有 decode 契约）。**FFI diff=0**（纯 Mojo，零 Rust/零 C）；e2e **768→785/785**（MB-1..MB-15 raw wire + MB-16/17 JSON access log 多字节）；raw fuzz 3602 请求 0 crash / bridge 516/0/4（无 Rust 改动）/ fmtool 35/0 / clippy 0（双 crate）/ bench 6 场景 0 errors / binary 5,188,032 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0；偏差=裸非 ASCII 查询值 `caf?`（决策-81）+ 非法字节头回显 U+FFFD（ADR-0059 §5）)
下一轮：P1/P2 open beads（非有限 float 渲染 / Content-Type 分派 parity / float 格式化 / SecurityScopes 对象 / OAuth2PasswordRequestForm 对象面 / CORS 裸 OPTIONS 超集 / multi-arch / asgi-shim）
*最后更新：2026-09-12（**决策-83 body 422 detail pydantic 逐字段对齐**（ADR-0058, Goal-0003 §1 矩阵 #4 body 422 detail 精度 / `fastapi_mojo-tew` bead 销账）：body 422 detail 逐字段对齐上游 pydantic v2 —— 约束类错误追加 `ctx`（house 键序 `loc,msg,type,input,ctx` ctx 末位 = ADR-0029 §3.7）；新增 `mo=N`（multiple_of）+ 数值首违序 `mo→le→lt→ge→gt`（同字段仅首个）；str 长度改 **codepoint**（`String should have at least/at most N characters`，修 ADR-0014 §4 字节计数偏差）；列表长度错 `too_short`/`too_long` + `actual_length` + 单复数 `item(s)` + **短路**元素校验；enum 带 `ctx.expected`；**`input` 片段按 JSON 类型渲染**（JSON 字符串值恒带引号）。附带 P0：① `numlit.parse_float_lax` 丢负号（`-5`→`5.0`）；② Mojo `String[byte=i]` 码点内部 assert 致 **server 崩溃**（form/cookie 值 + body 数组元素，实测修复）。约束应用层拆新 `body_constraints.mojo`（198 LOC；`body_validate.mojo` 523→333）。**FFI diff=0**（纯 Mojo，零 Rust/零 C）；e2e **733→768/768**（MX-1a..MX-13 28 项 + BY-3c..BY-3e）/ bridge 516/0/4 / fmtool 35/0 / clippy 0（双 crate）/ bench 6 场景 0 errors / binary 5,204,416 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0；偏差=house 键序 / CT 不参与 JSON 分派 / 非有限 float / 裸非 ASCII 查询值（ADR-0058 §5）
...
*最后更新：2026-09-12（**决策-82 body JSON 跨类型标量强制转换**（ADR-0057, Goal-0003 §1 矩阵 #4 body model 字段取值规范化 / `fastapi_mojo-g3p` bead 销账）：pydantic v2 model lax 跨 JSON 类型强制 —— 此前 body int/float/bool 字段只收「同族」JSON 类型（`int` 字段收 `7.0` → 422 int_parsing）；现 `int` ← JSON float 整值（`7.0`→7 / `1e2`→100 / `-3.0`→-3）/ bool（`true`→1 / `false`→0）/ string，非整 float → `int_from_float`，`null`/`{}`/`[]` → 裸 `int_type`；`float` ← JSON int/bool（`1`→1.0 / `true`→1.0）/ string，`null` → `float_type`；`bool` ← JSON 0/1/-0.0/1.0（`1`→true / `0`→false），其它数值 → `bool_parsing`，`null` → `bool_type`；数组元素逐元素同规则 + 规范化 JSON 文本重建（`[7.0,1e2,true,"7"]`→`[7,100,1,7]`）。新 `body_validate._coerce_body_scalar` / `_elem_json_type` / `_elem_type_err`；`_type_err` int/float/bool 改裸 `*_type`；嵌套/obj[] 经同一 `_validate_fields` 自动继承。**FFI diff=0**（纯 Mojo，零 Rust/零 C 改动）；e2e **709→733/733**（BY-1a..BY-9b 24 项）/ bridge 516/0/4（无 Rust 改动）/ fmtool 35/0 / clippy 0（双 crate）/ bench 6 场景 0 errors / binary ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0；偏差=string→int 非整仍 int_parsing / 回显 JSON 类型 / 非有限 float（ADR-0057 §3.5）
...
*最后更新：2026-09-12（**决策-81 lax 标量强制转换 + 规范化回显**（ADR-0056, Goal-0003 §1 矩阵 #2 参数取值规范化 / `fastapi_mojo-vg4` bead 销账）：pydantic v2 lax 补齐类型化输入面取值规范化 —— 此前 int/float/bool 成功路径原样回显请求字符串（`/calc/007/4`→`"007"`）；现 path/query/header/form/body JSON model 全场景按 lax 解析并回写规范化值。纯 Mojo：`numlit.parse_int_lax`（`007`→7 / `+5`→5 / `-0`→0 / `-0007`→-7 / `1_0`→10 / **整值小数 `2.0`→2 / `12.00`→12**；`2.5`/`2.`/`1e2`/`0x10`→int_parsing）+ `numlit.parse_float_lax`（`1.50`→1.5 / `1e3`→1000.0 / `007.0`→7.0 / `-0.0`→-0.0 / `1.`→1.0 / `.5`→0.5 / `1_0.5`→10.5 / 纯 int→`42.0`；`Float64`→String = CPython repr）+ `parse_bool_literal` pydantic lax 集（yes/y/t/on/1/TRUE → true；no/n/f/off/0/OFF → false）；新 `params_typed.canonicalize_typed_values` 在 `apply_query_extras` 后回写 path/query（含 alias 双键 + list 逐元素 CSV）；`form_params.apply_form_extras`（标量+list）/ `param_constraints_run.validate_headers_collect`（typed header）/ `body_validate._validate_fields`（含 JSON string→int/float/bool lax：`"007"`→7 / `"9.50"`→9.5）同款；str/enum/标量类型保持 raw（决策-79 echo 契约）。**FFI diff=0**（纯 Mojo，零 Rust/零 C 改动）；e2e **676→709/709**（CX-1..CX-21 + JS-3 改写）/ bridge 516/0/4（无 Rust 改动）/ fmtool 35/0 / clippy 0（双 crate）/ bench 6 场景 0 errors / binary ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0；偏差=body JSON float/bool→int 严格 / 非有限 float 渲染 / 回显 JSON 类型（ADR-0056 §3.5））
...
*最后更新：2026-09-12（**决策-80 请求 path 百分号解码**（ADR-0055, Goal-0003 §1 矩阵 #2 路径参数 / `fastapi_mojo-v50` bead 销账）：uvicorn 等价 —— 在 Starlette 路由前对 request target 做百分号解码；新纯函数 `params_query.url_decode_path`（`%XX` 单字节 / `+` 非空格 / 畸形 `%` 保留 / 解码字节 UTF-8，非法序列 → U+FFFD）；`get_path_slice` 后单点解码，router/static/param/日志/OTel/WS-upgrade 全链路一致；`a%20b`→`a b` / `%7Bx%7D`→`{x}` / `%E4%B8%AD`→`中` / `a+b`→`a+b` / **`a%2Fb`→404**（解码后分段）/ `%FF`→`�` / `%`→`%` / `%25`→`%` / 编码 `../` 静态 403；redirect Location 用 wire raw_path 拼装；**FFI diff=0**（纯 Mojo，零 Rust/零 C）；e2e **664→676/676**（PD-1..PD-11）/ bridge 516/0/4 / fmtool 35/0 / clippy 0 / bench 0 errors / binary ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0）
...
*最后更新：2026-09-12（**决策-79 pydantic 内建标量类型全量对齐**（ADR-0054, Goal-0003 §1 矩阵 #26 标量类型 / `fastapi_mojo-2wd` bead 销账）：补齐此前仅 `str/int/float/bool`（+ enum/list）的类型面 —— uuid/date/datetime/time/timedelta/decimal 6 类 pydantic 内建标量 path/query/header/body/form/list 全场景；纯 Mojo 解析器（`date_types`/`time_types`/`scalar_types`，含 byte-safe `bof()` 访问器规避 Mojo 1.0.0 `String[byte=]` 多字节 assert）；422 `type`/`msg`/`ctx`/`input` 逐字段对齐上游（uuid_parsing/date_parsing/date_from_datetime_inexact/datetime_from_date_parsing/time_parsing/time_delta_parsing/decimal_parsing/finite_number）；OpenAPI uuid/date/date-time/time/duration/decimal anyOf；**FFI diff=0**（零新符号/零新 crate/零 C）；e2e **609→664/664**（SC-1..SC-37）/ bridge 516/0/4 / fmtool 35/0 / clippy 0 / bench 0 errors / binary **5,151,168 B** ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0）
...
*最后更新：2026-09-12（**决策-78 GZip 中间件 Starlette 1.6.0 全量对齐**（ADR-0053, Goal-0003 §1 矩阵 #24 压缩 / `fastapi_mojo-mxs` bead 销账；**取代 ADR-0015 §3/§7**）：实读上游 `starlette/middleware/gzip.py`（1.6.0）+ ASGI 原始 message 探测（fastapi 0.141.1）修正决策-40 偏差 —— `Vary: Accept-Encoding` 在可压响应上**恒加**（`IdentityResponder`：未接受 gzip 也加；小体/排除/206/有 CE/HEAD 不加）/ client 判定 = **大小写敏感子串**（`GZIP` 不命中，`x-gzip`/`gzip;q=0` 命中）/ **13 项默认排除表**（`text/event-stream`/`image/*`/`video/*`/`audio/*`…）/ **streaming 可压**（more_body 分支，无 min_size 门）/ **FileResponse 可压**（≥min_size）/ `compresslevel` 6→**9**；`bridge/gzip.rs` 重写（`plan()→GzipPlan{vary,compress}` + `DEFAULT_EXCLUDE` + extra/Vary 头装配）+ `parse::accepts_gzip` + 三发送点接线 + `http2_response::send_streaming` 收 `&[u8]`；**FFI diff=0**；e2e **604→609/609**（GZ-1..GZ-10）/ bridge **509→516/0/4** / fmtool 35/0 / clippy 0 / bench 0 errors / binary **5,093,824 B** ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0）
下一轮：P1/P2 open beads（multi-arch / asgi-shim / SecurityScopes 对象 / OAuth2PasswordRequestForm 对象面 / CORS 裸 OPTIONS 超集）
*最后更新：2026-09-12（**决策-77 HEAD 请求统一仅头无体**（ADR-0052, Goal-0003 §1 矩阵 #1 路径方法 / fastapi_mojo-ki9 bead 销账）：修复内置 doc 路由/KIND_HTML/streaming 的 HEAD 响应携带 body 的 keep-alive 脱轨 bug —— `send_response` 单点 `include_body && !current_method_is_head()`（覆盖内置 doc/HTML/JSON/static/SSE/error/404/405, H1+H2 同享）+ `send_streaming_response` HEAD 仅头（H2 HEADERS 带 END_STREAM）；对齐上游 FastAPI（HEAD 响应头与 GET 相同、`Content-Length` = 完整体长度、body 空, RFC 9110 §9.3.2）；复用既有 `request::current_method_is_head()`；**FFI diff=0**；fmtool `headbody [path]`；e2e **600→604/604**（HD-1..4）/ bridge **507/0/4** / fmtool **35/0** / clippy 0（双 crate）/ bench 6 场景 0 errors / binary **5,089,704 B**（≤6 MiB）/ ldd 仅 libc / env-i 干净启动 / C=Python=orphans 0；边界=用户 GET 路由 HEAD 仍 200（既有文档化超集，上游 405）、非 FastAPI 内置路由 HEAD 亦随单点抑制无体）
下一轮：P1/P2 open beads（multi-arch / asgi-shim / SecurityScopes 对象 / OAuth2PasswordRequestForm 对象面 / CORS 裸 OPTIONS 超集）
*最后更新：2026-09-12（**决策-76 内置文档路由补全**（ADR-0051, Goal-0003 §1 矩阵 #16 OpenAPI / fastapi_mojo-redoc-oauth2-redirect-oxc bead 销账）：补齐上游 FastAPI 默认 4 条内置路由中缺失的 2 条（此前仅 `/openapi.json` + `/docs`）—— `/redoc`（ReDoc 页 `spec-url="/openapi.json"`）+ `/docs/oauth2-redirect`（Swagger OAuth2 回调页 `swaggerUIRedirectOauth2`）；`openapi.mojo` 增 `redoc_html`/`swagger_ui_oauth2_redirect_html` + `swagger_ui_html` 补 `oauth2RedirectUrl`；`http_server_final.mojo` 两条内置 GET 分支（HEAD 归一自动覆盖）+ import；**FFI diff=0**（纯 Mojo）；e2e **595→600/600**（DOC-1..4）/ bridge **505/0/4** / fmtool **35/0** / clippy 0（双 crate）/ bench 6 场景 0 errors / binary **5,089,704 B**（≤6 MiB）/ ldd 仅 libc / env-i 干净启动 / C=Python=orphans 0；边界=两条内置路由固定路径（`docs_url=/redoc_url=None` 未建模，与既有 `/docs` 一致）、HEAD 发送完整 body（上游 HEAD 长度 0 = 既存偏差）)
 下一轮：P1/P2 open beads（multi-arch / asgi-shim / SecurityScopes 对象 / OAuth2PasswordRequestForm 对象面 / CORS 裸 OPTIONS 超集）
*最后更新：2026-09-12（**决策-75 `Depends(yield)` teardown**（ADR-0050, Goal-0003 §1 矩阵 #9 依赖注入 / fastapi_mojo-dep-teardown-18z bead 销账）：依赖声明式 `_dep_teardown`（换行分隔命令）+ 实际派发登记（memo 命中不登记 → `use_cache=True` 每请求一次；子依赖先于父 = 解析完成序）+ 响应 flush **后逆序**执行（LIFO = 上游 request `AsyncExitStack` 关闭序），在 background tasks 之后 / 异常响应后仍执行（= 上游 `try/finally` 形式）；接入点 ×6（`(exc)`/`(stream)`/`(sse)`/`(file)`/`(redirect)`/主路径）；`_exec_one_cmd` 提取与 `_background` 共享；**FFI diff = 0**（内部函数签名新增 `mut teardowns`）；demo `/di-teardown` `/di-teardown-raise` `/di-teardown-twice`；e2e **591→595/595**（DT-1..4）/ bridge **505/0/4** / fmtool **35/0** / clippy 0（双 crate）/ bench 6 场景 0 errors / binary **5,077,416 B**（≤6 MiB）/ ldd 仅 libc / env-i 干净启动 / C=Python=orphans 0；边界=声明式 shell 命令（非运行期生成器）、teardown 恒执行（裸后置异常跳过不模拟）、命令不含换行）
 下一轮：P1/P2 open beads（multi-arch / asgi-shim / SecurityScopes 对象 / OAuth2PasswordRequestForm 对象面 / CORS 裸 OPTIONS 超集）
*最后更新：2026-09-12（**决策-74 `HTTPException(..., headers=...)` 自定义响应头**（ADR-0049, Goal-0003 §1 矩阵 #13 / fastapi_mojo-0n1 bead 销账）：异常响应可携带自定义头（上游 `HTTPException(status_code, detail, headers)` / exception_handler 返回带 headers 的 Response）—— Rust 新增 `send_text_response_status_extra(fd, status, body, extra)`（+1 FFI，JSON 面复用 `send_simple_response_extra`）；Mojo `exception_handlers.mojo` 增 `parse_exc_headers`（`;` 分隔 `TAG=H1|H2`，`|` 多头，`:` 校验，非法跳过 + 后者胜）/ `load_exc_headers`（路由 `_exc_headers` 覆盖 env `FASTAPI_MOJO_EXCEPTION_HEADERS`）/ `GuardResult.extra`；`resolve_exception_response` 命中 tag（或 `Exception` catch-all）填 extra；dispatch 异常分支按 extra 非空走 `*_extra` 入口；demo `/exc/headers`；**FFI +1** → e2e **587→591/591**（XH-14..17）/ bridge **505/0/4**（send_tests +2）/ fmtool **35/0** / clippy 0（双 crate）/ bench 6 场景 0 errors / binary **5,065,128 B**（≤6 MiB）/ ldd 仅 libc / env-i 干净启动 / C=Python=orphans 0；边界=声明式头表（非运行期 `headers=` dict）、头值不支持内嵌 `|`/`;`、未命中 tag（默认 500）无头）
 下一轮：P1/P2 open beads（multi-arch / asgi-shim / SecurityScopes 对象 / Depends yield teardown / CORS 裸 OPTIONS 超集）
*最后更新：2026-09-12（**决策-73 CORSMiddleware 全量等价**（ADR-0048, Goal-0003 §1 矩阵 #15 CORS / fastapi_mojo-utm bead 销账）：对齐 starlette 1.6.0 —— 真预检 200 + text/plain `OK` / 400 + `Disallowed CORS <origin,method,headers,private-network>` + 完整 preflight 头集（Vary/`*` + Allow-Methods + Max-Age + [Allow-Headers=sorted(SAFELISTED∪配置)] + [Credentials] + 动态 Allow-Origin/镜像/PNA）；新增 `FASTAPI_MOJO_CORS_ORIGIN_REGEX`（re.fullmatch, regex.rs `rgx_fullmatch`）/ `_EXPOSE_HEADERS` / `_PRIVATE_NETWORK`；`_METHODS=*`→ALL_METHODS / `_HEADERS=*`→预检镜像；method 匹配大小写敏感；普通响应 simple_headers 恒发 + echo `Vary: Origin`；PNA 走 request 全局（FFI 符号数不变）；H2 预检改直出 header_frames（修重复头 bug）；e2e **582→587/587**（CRS-1..9 + CRS2-1..4）/ bridge 503/0/4 / fmtool 35/0 / clippy 0（双 crate）/ bench 6 场景 0 errors / binary 5,052,840 B（≤6 MiB）/ ldd 仅 libc / env-i 干净启动 / C=Python=orphans 0；边界=裸 OPTIONS 仍 204 通配超集（既有文档化））
下一轮：P1/P2 open beads（multi-arch / asgi-shim / SecurityScopes 对象 / Depends yield teardown / CORS 裸 OPTIONS 超集）
*最后更新：2026-09-12（**决策-72 SSE ServerSentEvent 字段**（ADR-0047, Goal-0003 §1 矩阵 #10 响应类型 / sdo bead 销账）：对齐上游 `fastapi.sse.format_sse_event(*, data_str, event, id, retry, comment)` —— `streaming.mojo` 重写 150 LOC 全字段序列化（字段序 comment(`: <line>`)→event→data(逐行)→id→retry + `\n\n`；`_split_sse_lines` `\r\n`/`\r`→`\n` 且保留尾空串 —— `data="tail\n"`→`data: tail\ndata: \n\n`；data=raw_data 不 JSON 编码）；dispatch `KIND_SSE` 读 `_sse_event`/`_sse_id`/`_sse_retry`/`_sse_comment`（路由级）+ `build_sse_body_fields`；旧入口 `build_sse_body`/`format_sse_event(data)` 零变化（兼容）；**FFI diff=0**（纯逻辑）；e2e **577→582/582**（SF-1..5）/ bridge 500/0/4 / fmtool 35/0 / clippy 0（双 crate）/ bench 6 场景 0 errors / binary 5,044,576 B（≤6 MiB）/ ldd 仅 libc / env-i 干净启动 / C=Python=orphans 0；边界=字段为路由级（每事件同字段），非上游逐事件异构 `ServerSentEvent(event=.., id=..)`）
下一轮：P1/P2 open beads（multi-arch / asgi-shim / SecurityScopes 对象 / Depends yield teardown / CORS preflight 偏差）
*最后更新：2026-09-12（**决策-68 RedirectResponse**（ADR-0043, redirect-response bead 销账）：`KIND_REDIRECT` + `_redirect_url`/`_redirect_status` → Rust `send_redirect_response`（**FFI +1**）；上游 Starlette wire parity：**无 Content-Type**（media_type=None）+ `Content-Length: 0` + `Location`（safe set `:/%#?=@[]!$&'()*+,;` + `_.-~` 百分号编码，空格→%20）+ 空 body；默认 `307 Temporary Redirect`，支持 303/301/308；HTTP/2 HEADERS-only（无 content-type, content-length: 0, END_STREAM）；`redirect_quote`/`build_redirect_headers` 纯函数 + 4 单测；e2e **535→545/545**（RD-1..RD-10）/ bridge **500/0/4**（+4）/ fmtool **35/0** / clippy 0（双 crate）/ bench 6 场景 0 errors / binary **4,999,520 B**（≤6 MiB）/ ldd 仅 libc / env-i 干净启动 / C=Python=orphans 0；边界=非路由自动尾斜杠重定向（`redirect_slashes` 单独跟踪）、redirect 无 body 故不触发 GZip/用户中间件 response 面）
下一轮：P1/P2 open beads（multi-arch / asgi-shim）
*最后更新：2026-09-12（**决策-67 OAuth2 作用域**（ADR-0042, oauth2-scopes bead 销账）：`Security(get_current_user, scopes=[...])` / `SecurityScopes` 等价 — `_auth=oauth2` 路由声明 `_auth_scopes`（`;` 分隔）→ token `scope` claim 覆盖校验，缺 scope → **403 `Not enough permissions`** + `WWW-Authenticate: Bearer scope="<space-joined>"`；坏 token 401 同 scope 片段；无 Authorization 401 `Bearer`（框架先抛 parity）；token 路由 `_jwt_scopes` 签发 RFC 6749 `scope` claim；OpenAPI **securityScheme 修正为上游 oauth2 flows 形态**（`type:oauth2` + `flows.password.{scopes,tokenUrl}`，声明 `_oauth2_scopes`/`_oauth2_token_url`）+ operation `security` scope 数组；**FFI diff = 0**（复用 HMAC FFI）；`mojo run security_jwt.mojo` 自检扩 scope 向量；e2e **527→535/535**（OT-24/25 改 + OT-26..OT-33）/ ldd 仅 libc / env-i 干净启动 / binary ≤6 MiB / C=Python=orphans 0；边界=403 body 沿用 `{detail,status}`、scope 校验内建 `check_oauth2`、`_jwt_scopes` 静态声明、`_depends` 不接收 SecurityScopes 对象）
下一轮：P1/P2 open beads（multi-arch / asgi-shim）
*最后更新：2026-09-12（**决策-66 JSON response Rust 加速**（ADR-0041, json-rust bead 销账）：默认 Mojo serializer 不变，`FASTAPI_MOJO_JSON_SERIALIZER=rust` 仅对 ≥阈值（默认 64KiB 输入估算）的 flat response JSON object 启用 std-only Rust `JsonObjectWriter`；字段顺序仍由 Mojo `Dict` 拥有，输出与 `json.mojo` 字节兼容（`", "` / `": "` / `__nested__` raw / 控制字符契约），FFI +4（begin/add/finish/free，显式长度 + 输出 `malloc(n+1)+NUL`），失败回退 Mojo；配套 `span_to_str` ASCII fast path 批量构造，fmtool bench 场景新增 `data_file`→`hey -D`；escape 密集 1,000,014B JSON request（1,000,000 个待 escape 引号，n=200/c=1，两组均值）**91.27→113.35 req/s（+24.19%）**、avg latency **-19.44%**、P50 **-20.56%**；canonical bench 6 场景 0 errors（get_root_10k_100c=34,722.22 req/s）；e2e **521→527/527**（JR-1..6，主 server 全程 Rust path 回归）/ bridge **496/0/4**（+10）/ fmtool **35/0** / clippy 0（双 crate）/ string_builder 自检绿 / binary **4,962,656 B**（≤6 MiB）/ ldd 仅 libc / env-i 默认+opt-in health 200 / C=Python=orphans 0；边界=response-only flat object，request body parsing 仍在 Mojo，future bridge 内并发 dispatch 需 session-handle FFI）
下一轮：P1/P2 open beads（multi-arch / asgi-shim）
...
③ ~~GZip 不介入 file/streaming~~ → **已解除（决策-78, ADR-0053：streaming/FileResponse 均可压）**
