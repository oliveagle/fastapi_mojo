...
| 4 | 请求体 | Pydantic 模型 / dict / 嵌套 | ✅ 声明式 spec（决策-38）+ **pat=REGEX（决策-57, ADR-0032）** | **validator 自定义回调 = Mojo 无闭包（硬边界, ADR-0014）**；约束词表（gt/len/items/pat）= 等价形态 | — |
...
| 20 | Pydantic | 嵌套模型/Field 约束/validator/enum/自定义类型 | ✅ 嵌套+Field 约束+enum+**pat=REGEX（决策-57, ADR-0032）**；**无 validator closure = Mojo 无闭包（硬边界, ADR-0014）** | 元素级约束（P2） | §P2 |
