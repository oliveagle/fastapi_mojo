...
**已达成（v0.5.1 + 决策-31/32/33）**：
- 单 binary 2.8M，ldd 仅 libc，env -i 干净启动，e2e **147 项**，cargo 287 单测
- 已覆盖能力（见 §1 矩阵 ✅）：路由/路径参数/查询参数/类型化参数+422/JSON body/
  ...
  静态文件/HTML 响应/生产化(Docker+systemd+nginx)
...
| 17 | 安全 | HTTPBasic/HTTPBearer/APIKey/OAuth2/JWT/get_current_user | 🟡 Basic/Bearer/APIKey ✅（决策-34）；OAuth2/JWT P1 | 核心卖点已部分闭环 | **§P0 完成** |
| 18 | APIRouter | include_router(prefix/tags/dependencies) | ❌ | 核心 | §P1 |
| 19 | Lifespan | startup/shutdown (context manager) | ❌ | 核心 | §P1 |
