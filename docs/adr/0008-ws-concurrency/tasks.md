...
- ~~WS 路由 `{param}` pattern 匹配~~ → ✅ **已由 ADR-0009 落地**（WsRoute pattern + params 分派）
- ~~WS 鉴权~~ → ✅ **已由 ADR-0009 落地**（升级 query token；首帧/自定义头待后续）
- ~~事件队列背压策略~~ → ✅ **已由 ADR-0009 落地**（2*MAX_CONNS+64 结构上不可溢出 + 1008 防御）
- ~~重组缓冲内存策略~~ → ✅ **已由 ADR-0009 落地**（4KB 起步按需增长至 1MB）
- （ADR-0009 另发现并修复本 ADR 的 P0 bug：合并帧尾块丢失 —— 见 ADR-0009 §1）
