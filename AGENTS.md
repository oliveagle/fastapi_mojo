...
- **已决策-88**：**body 422 detail `input` 的 CPython/Starlette 反序列化-重序列化**（ADR-0063, Goal-0003 §1 矩阵 #4 422 detail 面 / 兑现 ADR-0060 §5 偏差行 + §8 #3）：上游 422 由 Starlette `JSONResponse` 渲染 (`json.dumps(ensure_ascii=False, separators=(",",":"), allow_nan=False)`)，pydantic error 的 `input` 是**已反序列化对象** → **重新序列化**；本实现此前回显**原始 JSON span**（形态差异，语义一致，ADR-0060 §5 记录）。差距实测：`[1, 2, 3]`→`[1,2,3]` / `1.50`→`1.5` / `1e2`→`100.0` / `"caf\u00e9"`→`"café"` / `"\u0041"`→`"A"`。新叶模块 `json_canon.mojo`（371 LOC，纯 Mojo，**FFI diff=0**，零新 crate/Rust/C）：`canon_json`（单遍递归下降校验+重序列化；失败/非有限 float/孤立代理 -> 原样回退）/ `_cpython_escape`（`\b\f\n\r\t` + `<0x20`→`\u00XX`）/ `json_string_literal`；数字复用决策-87 `atof_f64`+`fmt_f64_repr`；对象重复键后者胜（`{"a":1,"a":2}`→`{"a":2}`）。接线 `body_constraints._json_input_frag`/`_json_input_frag_typed`（string 分支）+ `body_validate._body_input` + 非 JSON CT 原始串。差分 fuzz（上游 `M{i:int,f:float,b:bool}` vs `/validate/cross`）**2961 组非规范 JSON `input`/status 0 不一致**；e2e **832→844/844**（JC-1..JC-12）/ bridge 516/0/4（无 Rust 改动）/ fmtool 35/0 / clippy 0（双 crate）/ 13 Mojo 自测全绿（+`json_canon_test`）/ bench 6 场景 0 errors / binary 5,220,800 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0
- **已决策-89**：**`response_model` `exclude_unset` / `exclude_defaults` / `by_alias` 语义 parity**（ADR-0064, `fastapi_mojo-fjt` bead 销账）：兑现 ADR-0062 §8 #3 / ADR-0063 §8 #2（Goal-0003 §1 矩阵 #11 response_model 剩余面）。上游 probe（fastapi 0.141.1）：plain `response_model` **注入声明默认值**（`{"name":"a"}` → `{"name":"a","price":10.0,"tax":0.0}`）；`response_model_exclude_unset` 剔**请求未提供**字段（显式提供=默认值仍保留）；`response_model_exclude_defaults` 剔**值等于声明默认**字段（未提供/显式均剔）；`response_model_by_alias`（默认 true）决定输出键用 alias 还是字段名。实现（声明式，延续 `handler.data`）：`_response_exclude_unset` / `_response_exclude_defaults` / `_response_aliases="name=alias;..."` / `_response_by_alias="false"`；模型**字段类型 + 默认值取自 `_body_schema`**（与请求校验同源，无新 spec 语言）；值解析优先 `body_<f>`（校验后规范值）再 `<f>`；数字/布尔/数组/对象渲染为 **raw JSON**（`__nested__:` 直通 —— float 字段输出 JSON number 而非字符串，对齐 pydantic），str/未知类型保持 JSON 字符串（既有行为零变化）。`validate_body_schema` 返回值扩为 **4 元组**（新增 `set_csv` = 显式提供顶层字段名，成功/422 路径均回传；embed 路径取内层同名集），dispatch 一行透传 `provided_csv`。应用序 include → exclude → unset/defaults → exclude_none；无 `_response_model` 仍整体 no-op。**FFI diff=0**（纯 Mojo，零 Rust/零 C）；e2e **844→859/859**（RMP-1..3 / RMU-1..5 / RMD-1..4 / RMB-1..3，逐条对齐上游 `/rn`·`/ru`·`/rd`·`/ba`·`/bn` probe）/ bridge 516/0/4（无 Rust 改动，+7 Mojo 自测断言）/ fmtool 35/0 / clippy 0（双 crate）/ 13 Mojo 自测全绿 / bench 6 场景 0 errors / binary 5,265,856 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0。偏差：必填字段缺失（上游 `ResponseValidationError` → 500）本实现跳过；alias 走声明式 `_response_aliases` 表（非 Pydantic `Field(alias=)` 自动映射）。
*最后更新：2026-09-13（**决策-89 response_model exclude_unset/exclude_defaults/by_alias**（ADR-0064, Goal-0003 §1 矩阵 #11 response_model）：上游 plain 注入声明默认值 / exclude_unset 剔未提供 / exclude_defaults 剔等于默认 / by_alias 输出键（默认 true）；声明式 `_response_exclude_unset`/`_response_exclude_defaults`/`_response_aliases`/`_response_by_alias`，模型类型+默认取自 `_body_schema`；`validate_body_schema` 4 元组 `set_csv`（显式提供字段）透传 dispatch；数字字段渲染 JSON number；**FFI diff=0**（纯 Mojo）；e2e **844→859/859**（RMP/RMU/RMD/RMB 15 例）/ bridge 516/0/4 / fmtool 35/0 / clippy 0 / 13 Mojo 自测 / bench 0 errors / binary 5,265,856 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0）
...
*最后更新：2026-09-13（**决策-88 body 422 `input` CPython/Starlette 反序列化-重序列化**（ADR-0063, Goal-0003 §1 矩阵 #4 422 detail）：上游 `input` = 已反序列化对象重新序列化（去空白/数字规范化/字符串解码重编码/重复键后者胜）；新叶 `json_canon.mojo`（纯 Mojo，**FFI diff=0**）；接线 `_json_input_frag`/`_json_input_frag_typed`/`_body_input`；差分 fuzz 2961 组 0 不一致；e2e **832→844/844**（JC-1..JC-12）/ bridge 516/0/4 / fmtool 35/0 / clippy 0 / 13 Mojo 自测 / bench 0 errors / binary 5,220,800 B ≤6MiB / ldd libc-only / env-i 干净 / C=Python=orphans 0）
