...
# 集成测试（单一 binary 端到端，604 项检查含 HEAD 无体（HD-1..4）、内置文档路由（/redoc + /docs/oauth2-redirect）、Depends(yield) teardown、异常响应自定义头、Rust JSON serializer、OAuth2 scopes、RedirectResponse/redirect_slashes、SSE ServerSentEvent 字段、CORS 全量等价、HTTPDigest/securitySchemes、OpenIdConnect/AuthorizationCode、TLS/HTTPS、
